...
const ReportInternalFailures3 = () => {
    ...
    const validarCampos = () => {
        const nuevosErrores = {};
        const camposObligatorios = ["cliente", "placa", "marca", "tipo", "equipo", "kilInicial", "kilFinal", "falla", "trabajoRealizado"];
    ...
    const handleSubmit = async () => {
        try {
            const response = await axios.post('http://localhost:3000/reportes-internos', formData);

            if (response.status === 201 || response.status === 200) {
                alert('¡Reporte enviado con éxito!');
                console.log('Respuesta del backend:', response.data);
                // Limpiar formulario o redirigir si es necesario
            } else {
...
return (
    <form
        ...
        className="p-6 max-w-6xl mx-auto space-y-6 bg-white shadow rounded"
    >
        ...
        <section>
            ...
            />
            {errors.falla && (
                <p className="text-red-500 text-sm mb-2">{errors.falla}</p>
            )}
            ...
            {errors.trabajoRealizado && (
                <p className="text-red-500 text-sm">{errors.trabajoRealizado}</p>
            )}
        </section>
        ...
        <section>
            ...
            {errors.accesorios && (
                <p className="text-red-500 text-sm mt-1">{errors.accesorios}</p>
            )}
        </section>
            ...
            {formData.repuestos.map((repuesto, index) => (
                <div key={index} className="grid grid-cols-3 gap-2 mb-2">
                    <input
                        ...
                        onChange={(e) =>
                            handleRepuestoChange(index, "nombre", e.target.value)
                        }
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            handleRepuestoChange(index, "cantidad", e.target.value)
                        }
                        className="border p-2 rounded"
                    />
                    <input
                        ...
                        onChange={(e) =>
                            handleRepuestoChange(index, "precio", e.target.value)
                        }
                        className="border p-2 rounded"
                    />
                </div>
        ...
        {/* Áreas del camión (nuevo componente aquí) */}
                                    ...
                                    {itemsRevision.map((item) => (
                                        <label key={item} className="flex items-center gap-2">
                                            <input
                                                ...
                                                onChange={() =>
                                                    toggleRevisionItem(i, lado, item)
                                                }
                                            />
                                            <span>{item}</span>
                                        </label>
