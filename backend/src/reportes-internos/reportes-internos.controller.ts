...
export class reportesInternosController {
  ...
  async crear(@Body() dto: CrearReportDto) {
    console.log("📩 DTO recibido en /reportes-internos:", dto); // 👀 Log de entrada
  ...
  @Get(':id') // ⚠️ aquí corregí: antes estaba mal puesto como 'id'
  ...
  async crearFull(@Body() dto: CrearReportDto) {
    console.log("📩 DTO recibido en /reportes-internos/full:", dto); // 👀 Log de entrada

    const nuevoReporte = await this.service.crearReporte(dto);
    return {
      mensaje: 'Reporte completo creado correctamente',
